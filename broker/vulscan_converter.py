...
def vulscan_converter(filename):
    ...
    if data["nmaprun"]["runstats"]["hosts"]["@up"] != '0':
        ...
        if "hostnames" in data["nmaprun"]["host"]:
            if data["nmaprun"]["host"]["hostnames"] is not None:
                if "hostname" in data["nmaprun"]["host"]["hostnames"]:
                    if data["nmaprun"]["host"]["hostnames"]["hostname"] is not None:
